...
U 是 $M×M$ 的单位正交矩阵，其列向量称为左奇异向量，代表数据的空间模式。

Σ 是$M×N$ 的对角矩阵，其对角元素是奇异值，代表各模式的能量。

V 是 $N×N$ 的单位正交矩阵，其列向量称为右奇异向量，代表时间模式。

**3 提取主要POD模态/模式**

选择 $r $个最大的奇异值对应的左奇异向量，构成矩阵 $U_r$
​
 $U_r = [u_1,u_2,...u_r]$

其中 $r$ 是远小于$M$ 和 $N$ 的数，通常根据累积能量百分比来选取。

**4 构建降阶模型**

降阶模型的表示如下：
$ {\tilde{x(t)}} \approx U_{r}{a}(t) $



现在，对于新的时间点 $t$，我们收到了新的状态数据 $x(t)$。我们计算它在$U_r$子空间中的投影，得到：

$a(t)=U_r^Tx(t)$
带入上面的公式即可得到重构后的值

上面的约等号表示${\tilde{x(t)}}$ 是 $x(t)$在$U_{r}$子空间中的近似。
