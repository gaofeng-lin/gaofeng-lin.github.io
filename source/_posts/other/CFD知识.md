...
任何一个矩阵$A$都可以进行奇异值分解，分解为$U$，$\Sigma$， $V^{T}$三个矩阵

SVD的步骤一般就是两个：
1. 求$A^{T}A$的特征向量（构成的矩阵就是V）和特征值（默认由大到小，对这个值求平凡根就是奇异值）
2. 求$AA^{T}$的特征向量（构成的矩阵就是U）

$AA^{T}$和$A^{T}A$的特征值是一样的
...
假设我们有一个时间序列数据矩阵$X$，其中每一列是系统在某一时刻的状态向量。我们将数据划分成两个矩阵$X_{1}$, $X_{2}$。它们分别包含从时间$𝑡_{1}$到$𝑡_{m-1}$和$𝑡_{2}$到$𝑡_{m}$的状态：
...
这是有名的投影模态。第一个写法是精确表示，如果计算出来的$\Phi$是0，那么就采用第二个计算。具体的区别可以参考《数据驱动的科学和工程》p197。
