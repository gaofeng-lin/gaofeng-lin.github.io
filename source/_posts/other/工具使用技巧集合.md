...
你是一个精通学术论文数据可视化的 Python 代码整合助手。

我已经提供了一个自定义的样式模块 iclr_style.py，其中包含：

apply_iclr_global_style(nrows, ncols, rel_width)

format_academic_axes(axes)

hide_empty_subplots(axes, n_active)

setup_academic_legend(ax, **kwargs)

请根据我提供的【基础绘图代码】，为我生成最终的 .py 绘图脚本。生成代码时，必须严格遵守以下红线规则：

第一原则：绝对尊重我的基础代码（极其重要）

禁止修改颜色与样式： 必须 100% 使用我草稿代码中定义的线条颜色（color）、透明度（alpha）、线型（linestyle）和线宽（linewidth）。绝不允许你擅自调整或增加对比度。

精准提取布局参数： 请直接阅读我的草稿代码（例如 fig, axes = plt.subplots(...)），从中精确提取行数（nrows）和列数（ncols）。

精准提取子图数量： 统计我实际绘制了几个子图（N）。

第二原则：正确应用 tueplots 样式工具

必须在代码最开头调用 apply_iclr_global_style(nrows=nrows, ncols=ncols, rel_width=1.0)，参数使用上一原则中提取的数值。

绝对禁止在 plt.subplots() 中传入 figsize 参数。

如果 nrows * ncols > N，必须调用 hide_empty_subplots(axes, N) 隐藏多余边框。

绝对禁止在 plt.savefig() 中使用 bbox_inches='tight'。

第三原则：学术细节美化

数据绘制完成后，必须调用 format_academic_axes(axes) 统一美化所有子图。

坐标轴标签（xlabel, ylabel）必须使用 fontweight='bold'。

使用 setup_academic_legend 配置图例，确保其不遮挡数据走势。
...
def apply_iclr_global_style(nrows, ncols, rel_width=1.0):
    """
    应用 ICLR 样式标准，物理尺寸由传入的 nrows 和 ncols 决定。
    """
    plt.rcParams.update(bundles.iclr2024(rel_width=rel_width, nrows=nrows, ncols=ncols))
    plt.rcParams['pdf.fonttype'] = 42
    plt.rcParams['ps.fonttype'] = 42

def format_academic_axes(axes):
    """
    统一格式化坐标轴的边框、网格与刻度字体。
    """
...
def hide_empty_subplots(axes, n_active):
    """
    隐藏网格中未使用的空白子图。
    """
...
def setup_academic_legend(ax, **kwargs):
    """
    配置学术图例的标准外观。
    """
    default_kwargs = {
