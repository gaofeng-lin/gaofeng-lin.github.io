...
### matplotlib绘图以适应latex排版

学术投稿一般都是latex，但是有些图是需要用matplotlib绘图，但是过往绘制出来的图，放到latex里面，会出现图片过大或过小的问题，之前我都是使用```width=0.85\textwidth```进行缩放，但是这就其实不好，会导致**图片中的所有元素（线条、颜色、文字）都被一起缩放**，在生成的pdf里面看起来就很奇怪。所以解决办法是：**永远在绘图时就确定最终物理尺寸**。

一旦确定了画布宽度，就永远不要在 LaTeX 中对图片进行任何缩放。

为了解决这个问题，可以依靠tueplots来帮助我们解决这个问题，针对ICLR排版（其他顶会也有），自动调整图片的宽度/字体大小等。

所以后续的做图方法，可以先把图的大致布局、配色等弄好。然后使用下面这个py文件，可以发给大模型处理。**只需要思考 "我要展示什么数据" 和 "怎么展示最清楚"，而所有技术细节都由大模型来处理。**。如果还是有些不符合要求，手动调整下即可

```
# iclr_plot_config.py - 你的个人ICLR绘图配置库
import matplotlib.pyplot as plt
from tueplots import bundles

def iclr_style(nrows=1, ncols=1, rel_width=1.0, aspect_ratio=None):
    """
    所有参数都经过优化，99%的情况下不需要修改
    """
    # 基础样式（来自tueplots）
    plt.rcParams.update(bundles.iclr2024(rel_width=rel_width, ncols=ncols, nrows=nrows))
    ...
    if aspect_ratio is None:
        aspect_ratio = {
            (1, 1): 0.618, (1, 2): 0.6, (1, 3): 0.6,
            (2, 1): 0.8, (2, 2): 0.7, (2, 3): 0.7,
            (3, 2): 0.8, (3, 3): 0.85
        }[(nrows, ncols)]
    
    # 设置精确物理尺寸
    TEXT_WIDTH = 5.5  # ICLR正文宽度（英寸）
    plt.rcParams['figure.figsize'] = (
        TEXT_WIDTH * rel_width, 
        TEXT_WIDTH * rel_width * aspect_ratio
    )
    
    # 自动设置最佳子图间距
    plt.rcParams.update({
        'figure.subplot.hspace': {1: 0.2, 2: 0.35, 3: 0.4}[nrows],
        'figure.subplot.wspace': {1: 0.15, 2: 0.25, 3: 0.25}[ncols],
        'figure.subplot.top': 0.88 if (ncols>=3 or nrows>=2) else 0.92,
        'figure.subplot.bottom': 0.12,
        'figure.subplot.left': 0.1,
        'figure.subplot.right': 0.98,
    })

def add_bold_ticks(ax):
    """统一加粗所有刻度数字"""
...
# 任务说明
我会给你一段 Python matplotlib 绘图代码（业务代码）。请你仅对样式部分做标准化改造，所有数据处理、绘图逻辑、配色方案、线条样式、注释内容必须 100% 原封不动保留。
样式统一基于我固定的配置库iclr_plot_config.py实现，你不需要知道配置库的内部代码，只需严格按照下面的接口约定调用。

# 配置库接口约定（必须严格遵守）
配置库提供两个函数，直接导入使用即可：
iclr_style(nrows=行数, ncols=列数, rel_width=1.0, aspect_ratio=None)
作用：一键设置 ICLR 标准的画布尺寸、字体大小、子图间距、导出参数
参数：nrows是子图行数，ncols是子图列数，必须根据代码自动识别填写
可选参数：aspect_ratio用于调整高宽比，默认不填即可
add_bold_ticks(ax)
作用：给指定子图的刻度数字加粗
用法：对每个子图 ax 调用一次
...
# 强制转换规则
✅ 必须移除的内容（一个都不能留）
所有figsize=(...)参数（从plt.subplots()中删除）
所有fontsize=...参数（从所有标签、标题、图例、刻度中删除）
所有plt.subplots_adjust(...)、fig.subplots_adjust(...)调用
所有bbox_inches='tight'参数（从plt.savefig()中删除）
所有手动设置plt.rcParams字体、尺寸、间距的代码
✅ 必须 100% 保留的内容（绝对不能改）
所有数据加载、计算、预处理代码
所有绘图逻辑（plot/bar/scatter 等函数的调用和参数）
所有颜色设置（color、facecolor、alpha 等）
所有线条样式（linewidth、linestyle、marker 等）
所有坐标轴范围（xlim、ylim）
所有注释、箭头、文本标注（annotate、text 等）
所有网格设置（grid 的样式和透明度）
所有变量名、函数名、代码结构和缩进
✅ 必须添加的内容
代码最开头添加导入语句：
python
运行
from iclr_plot_config import iclr_style, add_bold_ticks
在plt.subplots()之前，添加样式初始化：
python
运行
iclr_style(nrows=X, ncols=Y)
其中 X、Y 自动识别代码中子图的实际行数和列数
所有ax.set_xlabel()、ax.set_ylabel()调用中，添加fontweight='bold'参数
对每一个子图 ax，都调用一次add_bold_ticks(ax)（放在遍历子图的循环里最合适）
plt.savefig()仅保留文件名和format='pdf'，删除其他所有参数

# 特殊布局处理规则
图例处理：
当子图总数 ≥ 4 时，自动删除所有ax.legend()调用，改为在所有绘图结束后添加顶部统一图例：
python
运行
...
fig.legend(handles, labels,
           ...
           facecolor='white',
           edgecolor='lightgray')
子图总数 < 4 时，保留原代码中的图例位置和设置，仅删除 fontsize 参数
子图编号：
禁止自动添加 (a)(b)(c) 子图编号，除非我明确要求
原代码中已有的标题、编号全部保留，不做增减

# 输出要求
只输出修改后的完整 Python 代码，不要任何解释、说明、注释
不要改变原代码的段落结构和空行
不要添加任何额外的功能或美化
代码必须可以直接运行，无语法错误
...
我自己目前大概总结出来了一套方法，以后如果要把论文导入进去，要确保参考文献不出错的情况下，那么首先要保证这个论文的**条目信息**没有错误，右键-》生成条目报告，就能看到相关信息，就能知道这个这个论文却不缺少内容。

然后，因为只传入pdf，然后自动抓取，一般会缺少很多信息。所以，最好的方法是使用bibtex，次选方法是doi。因为有的论文还没有doi。

关于bibtex，可以不用去官网看，因为有可能会比较慢。如果要批量导入，那么直接浏览器搜索：bibtex 论文名称。 这个时候ai可能会为你查找到bibtex，不过有的可能用的是arxiv的，这个时候可能需要自己去官网下载。然后把多篇论文的bibtex复制到一个bib文件，然后直接导入。这个时候批量生成了条目，然后全选+右键，查找全文，就能获取pdf。但是这个时候，有的可能没找到pdf，原因大概率是bibtex有问题，缺少了url链接。或者这个论文是IEEE等网站，没法直接下载。如果是缺少了链接，那么就去官网找bibtex就好了；如果是IEEE，那需要单独下载pdf，手动添加。

### ZoteroV9需要配置的插件

直接从官网下载就好，一共有三个需要下载：

- Better Bibtex: 9.0.23
- Translate for Zotero: 2.4.4
- Zotero Attanger: 1.4.1

Zotero Attanger类似之前的ZotoFile。主要是解决Zotero存储空间不够的问题，因为ZotoFile在高版本的Zotero中不支持，所以重新弄了一个新插件，但是本质是一样的。需要注意的地方如下图所示：两个目录还是和之前一样，Onedrive下的ZoteroPage

![](https://cdn.jsdelivr.net/gh/gaofeng-lin/picture_bed/img1/Snipaste_2026-05-13_18-18-52.png)

另一个需要配置的地方是：常规-》文件重命名-》文件名模板

![](https://cdn.jsdelivr.net/gh/gaofeng-lin/picture_bed/img1/Snipaste_2026-05-13_18-22-08.png)
...
Markdown默认是没有编号，生成的pdf也没有目录。为了美观，可以添加上。

Vscode插件：Markdown All in One

使用方法：

Ctrl+Shift+P 或者 View->Command Palette。

然后选择下面这两个进行编号生成和目录生成：

![](https://cdn.jsdelivr.net/gh/gaofeng-lin/picture_bed/img1/Snipaste_2025-11-03_15-52-12.png)

![](https://cdn.jsdelivr.net/gh/gaofeng-lin/picture_bed/img1/Snipaste_2025-11-03_15-52-34.png)
...
输入第一行公式，第一行公式尾部输入#，按回车。如果不行的话就按 shift+回车

如果公式没法居中，鼠标放置在公式任意位置，右键，取消勾选扩展到页/列宽度；

鼠标放置在整个公式的尾端（非单行的尾端），输入 #(1) ，并回车，1为公式编号；

在每行等号后输入 & ，进行对齐，或鼠标放置等号后右键，点击在此字符处对齐。
...
![](https://cdn.jsdelivr.net/gh/gaofeng-lin/picture_bed/img1/%E9%80%86%E5%85%89%E6%8B%8D%E6%91%84%E4%BA%BA%E5%83%8F.png)
