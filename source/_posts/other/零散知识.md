...
$ \begin{array}{c}{{\frac{d}{d x}(x^{2})=\frac{f(x+d x)-f(x)}{d x}}}\\ {{=\frac{(x+d x)^{2}-x^{2}}{d x}}}\\ {{=\frac{x^{2} + 2x{d x} + dx^{2} -x^{2}} {d x}}}\\ {{=\frac {2x{d x} + dx^{2}}{d x}}}\\ {{=2x+d x}}\\ {{= 2x}} \end{array} $
...
**上面的操作完成后一般都没问题，如果发现部分公式能渲染，部分不行。原因可能是因为_或者其他符号，只需在前面加一个反斜杠就好，变成下面这样：**
...
Introduction里面就要把idea和解决方法说出来，但是不要太细。
