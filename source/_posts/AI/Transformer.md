...
在 PatchTST 中，一个 Token（Patch）是指：单个变量（Channel）中，连续的 $P$ 个时间点的数据。

- 维度reshape: 模型不再认为这是 "32 个包含 7 变量的序列"，而是认为这是 "32 * 7 = 224 个单变量序列"。$$(32, 96, 7) \rightarrow (32 \times 7, 96, 1) \rightarrow (\mathbf{224}, 96, 1)$$
  对于 Transformer 来说，它正在处理 224 条独立的线，它根本不知道这 224 条线里，其实每 7 条是属于同一个时刻的。
- patch: 现在我们只看这 224 条线中的一条（长度 96）。我们对其进行 Patch 操作。$96 / 16 = 6$ 个 Patch。$$(224, 96, 1) \rightarrow (224, 6, 16)$$
  这个时候token_num变为6，token_len变为16。
- Embedding:把长度为 16 的原始数据投影到 Transformer 的隐层维度 $D$（比如 $D=128$）。


**计算量从 $O(N^2)$ 降到了 $O((N/P)^2)$，这是指数级的优化，这也是为什么patchtst说自己减小了显存占用、训练速度加快、而且包含局部语义信息的原因**。
...
这时候每个token就是一个时间点的数据，每个时间点的长度为D。那么这个时候计算复杂度就是就为$O(N^2D)$，**也就是我们常说的计算复杂度和输入序列长度的平方成正比**。
