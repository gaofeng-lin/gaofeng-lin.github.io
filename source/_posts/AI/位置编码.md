...
置换不变：随意调换输入中词的位置，并不会让注意力权重发生变化，注意力层的整个结果维持不变。具体来说，每一个词向量计算结果不变，仅仅是词向量在输出矩阵的排列随着词和词位置互换而对应调整了一下。
...
${C}_{t}={S}_{t}{V}$
${C}_{t}^{\prime}={S}_{t}^{\prime}{V}^{\prime}=({S}_{t}P^{\dag})(P{V})={S}_{t}{V}={C}_{t}$
