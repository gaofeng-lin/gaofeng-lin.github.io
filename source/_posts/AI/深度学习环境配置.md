...
## 运行深度学习代码，使用gpu1报错，使用gpu0正常运行

训练的时候能正常运行，但测试的时候报错，报错信息如下：

```
Traceback (most recent call last):
  File "/root/FilterTS/run.py", line 140, in <module>
    exp.train(setting)
  File "/root/FilterTS/exp/exp_main.py", line 203, in train
    self.model.load_state_dict(torch.load(best_model_path))
  File "/root/.pyenv/versions/llmtpf/lib/python3.10/site-packages/torch/serialization.py", line 1025, in load
    return _load(opened_zipfile,
  File "/root/.pyenv/versions/llmtpf/lib/python3.10/site-packages/torch/serialization.py", line 1446, in _load
    result = unpickler.load()
  File "/root/.pyenv/versions/llmtpf/lib/python3.10/site-packages/torch/serialization.py", line 1416, in persistent_load
    typed_storage = load_tensor(dtype, nbytes, key, _maybe_decode_ascii(location))
  File "/root/.pyenv/versions/llmtpf/lib/python3.10/site-packages/torch/serialization.py", line 1390, in load_tensor
    wrap_storage=restore_location(storage, location),
  File "/root/.pyenv/versions/llmtpf/lib/python3.10/site-packages/torch/serialization.py", line 390, in default_restore_location
    result = fn(storage, location)
  File "/root/.pyenv/versions/llmtpf/lib/python3.10/site-packages/torch/serialization.py", line 265, in _cuda_deserialize
    device = validate_cuda_device(location)
  File "/root/.pyenv/versions/llmtpf/lib/python3.10/site-packages/torch/serialization.py", line 256, in validate_cuda_device
    raise RuntimeError('Attempting to deserialize object on CUDA device '
RuntimeError: Attempting to deserialize object on CUDA device 1 but torch.cuda.device_count() is 1. Please use torch.load with map_location to map your storages to an existing device.
```

**错误原因：模型保存时记录了原始 GPU 设备编号，加载时未正确映射到当前可用设备。**
...
```
# 方式1：先加载到CPU，再移动到目标设备（兼容性最好）
checkpoint = torch.load(best_model_path, map_location='cpu')  # 强制先加载到CPU
self.model.load_state_dict(checkpoint)
self.model.to(self.device)  # 再移动到当前目标GPU（假设self.device已正确设置）
```
