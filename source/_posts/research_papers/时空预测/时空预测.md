...
- AMD：根据MDM的输出结果，对前top-k个权重和后top-k的个权重进行区分；基于权重，使用MOR，对DDI的结果进行聚合。
  - 验证思路：
    - RandomOrder：没有使用 MDM 输出的真实 $u$（包含时序特征的 embedding）作为输入来生成权重，而是将其替换为了随机的序列/向量。（这个是问的gemini，有可能不正确，我也不是很理解这个意思）
    - Average Weight：每个专家的权重是一样的。原文采用的是TP-Selector 的动态打分机制（Softmax/Top-K 等）
...
而batchnorm，是针对每个变量处理，也就是同一个变量，要处理样本*时间点（32*96）个数据点，进行归一化。

# FilterNet: Harnessing Frequency Filters for  Time Series Forecasting

NeurIPS[2024]

Tranformer-based对高频信号很脆弱，计算效率不行，全频谱利用不行。

idea：三个不同频率组成的纯净信号下，itransformer效果很差，无法利用全频谱信息。 相比之下，在信号处理领域，频率滤波器具有频率选择性、信号调理、多速率处理等诸多优良特性。
...
传统的时序分析方法主要集中在时域，但往往不能捕获周期信息。最近越来越多的方法开始将频域应用到时间序列。虽然有进步，但是这些方法有时**不能有效区分各种频率的重要性，平等地对待所有成分，忽略重要的先验周期信息和变量之间的依赖可能导致过拟合**。
...
1. 时域到频域的嵌入 ： 将输入序列通过快速傅里叶变换 (FFT) 映射到频域。
2. 双路并行滤波提取特征：
   1. 动态跨变量滤波：捕获各个变量之间“同步演变”的动态频率
   2. 静态全局滤波：捕获那些长期存在的自然周期（如日、周、月等节律）。它通过在整个训练集上统计出振幅最高的 $K$ 个主导频率，并构建静态带通滤波器 。
3. 特征聚合 (Aggregation)： 将原始频域嵌入、动态滤波结果和静态滤波结果进行加权融合。
4. 频域到时域的投影 ： 将融合后的复数频域特征，分别处理实部和虚部，最终映射回时域输出预测结果 。
...
（1）最近的实证研究（Ye et al., 2024；Zhang et al., 2024）表明，不同频率分量的重要性在不同场景中有所不同，每个分量对预测性能可能有利或有害，具体取决于具体的上下文，这表明**简单地丢弃某些频率分量或独立处理它们可能不是最优的**。**然而，尽管最近的研究引入了频域处理，但它们主要侧重于独立处理不同的频率分量，而没有探索跨频率相互作用的建模**。
（2）**尽管在时间戳建模方面进行了这些尝试，但时间戳固有的周期性特征仍未得到充分探索**。为了充分利用时间戳的潜力来增强性能，探索它们的周期性模式成为一个有前途的方向。

两个专门分支增强长期时间序列预测的新颖框架。

首先，我们提出了**跨频率交互**（CFI）分支，它能够对不同频率分量进行单独建模，同时通过精心设计的特征融合机制捕获它们的交互。因此，它保留了长期演化模式和短期动态特征，并提供跨时间尺度的全面表示。

其次，我们**设计了周期感知时间戳建模**（PTM）分支。它根据固定周期长度将一维时间戳序列转换为二维张量。通过2D卷积运算，它捕获了周期内依赖性和周期间相关性，增强了预测的时间上下文并改善了预测结果。
...
![](https://cdn.jsdelivr.net/gh/gaofeng-lin/picture_bed/img1/Snipaste_2026-03-22_14-24-02.png)

### CFI

**动机**： 现有方法要么忽略频域信息，要么独立处理不同频率成分，但没有建模频率之间的交互关系。

对归一化后的数据，进行下面的处理：
| 步骤 | 操作 | 目的 |
|------|------|------|
| 频率划分 | 以中位频率为阈值，分离为低频 FL 和高频 FH | 低频携带基础模式和趋势，高频反映短期动态 |
| 特征提取 | 提取实部和虚部，拼接后通过线性层 | 分别处理对称和不对称模式 |
| 耦合交互 | L 层耦合网络，使用公式(14)(15) | 核心创新：建模高低频之间的相互影响 |
| 频率重组 | 拆分实虚部，重建复数，IDFT 逆变换 | 回到时域得到预测 |
...
![](https://cdn.jsdelivr.net/gh/gaofeng-lin/picture_bed/img1/Snipaste_2026-02-03_15-38-16.png)

核心流程：

- Revin归一化，rfft
- 过滤部分高频频率。过略的方法是：
  - 对数据进行fft后把幅值最高的那个作为主频/基频（Dominant Frequency/Base Frequency），设置截止频率（COF）为基频的第 $n$ 个倍数（即第 $n$ 个谐波）。从论文图3来说，3(b)是6倍，那么截断频率就会比较高，那么保留的频率会偏多，反向fft后，计算的mse就会越小。
  - gemini分析，这样做是因为：根据傅里叶分析，任何复杂的周期性波形都可以看作是一个基频的正弦波与多个不同频率、不同振幅的正弦波（即谐波）叠加而成的。基频决定了信号的大致周期，而谐波决定了波形的细节（比如尖峰、转折或波动的剧烈程度） 。直接按频率高低过滤太粗暴，应该根据信号的“结构”来过滤。所以设置截止频率（COF）为基频的第 $n$ 个倍数（即第 $n$ 个谐波）。
-  用复数线性层进行插值：
   -  假设：$batch\_size=32$, $seq\_len=96$, $channel\_num=7$, $pred\_len=336$
   -  归一化后rfft，输入长度为 $N$ 时，rFFT 输出长度为 $N/2 + 1$ 。$96 / 2 + 1 = 49$。维度变为：$(32, 49, 7)$
   -  低通滤波：只保留前 $K$（dominance_freq）个频率分量。维度变为：$(32, K, 7)$。注意，这里的 $K$ 必须 $\le 49$
   -  复数线性层插值：为了适应 nn.Linear，维度转为 $(32, 7, K)$。 从 $K$ 映射到 $int(K \times \eta)$。这里即 $K \to int(K \times 4.5)$。维度变为：$(32, 7, 4.5K)$，随后转回 $(32, 4.5K, 7)$。
   -  零填充：为了得到 $L_o = 432$ 的时域输出，这里L_o是输入+输出的长度，irFFT 需要的频谱长度为 $432 / 2 + 1 = 217$ 。创建一个全 0 张量 $(32, 217, 7)$，将插值后的 $4.5K$ 个分量填入低频部分 。剩下的位置填充0。
   -  反向傅里叶变化+能量补偿+归一化：
      -  这里说一下能量补偿，代码里面为：```low_xy=low_xy * self.length_ratio # energy compemsation for the length change```，我也不是很懂，按照gemini的说法：当你将长度为 $L_i$（96）的序列通过插值和补零变为长度为 $L_o$（432）的序列时，频谱的能量密度发生了变化。如果你在频域增加了序列长度（补了更多的 0），直接进行逆变换会导致输出的时域信号振幅（能量）变小。length_ratio（即 $\eta = L_o/L_i$）作为补偿系数，是为了确保生成的 $L_o$ 长度信号在振幅量级上与原始输入的 $L_i$ 信号保持一致 。如果不做这个乘法，预测出来的曲线会在数值上比原始输入“矮”一大截。


（表4）COF数值的设置，对效果的影响非常非常小，即便增加截至频率，也不一定会增加效果，甚至会变差，而且截至频率变高，意味着你要处理的输入变长，计算资源需要的更高。增加输入长度倒是能明显提升效果。**但说实话，它并没有验证，截断相比不截断是否有提升**。
...
与传统时序预测以时间点作为token不同，VarDrop是以变量作为token，那么注意力复杂度就是**变量数长度的平方成正比**
...
![](https://cdn.jsdelivr.net/gh/gaofeng-lin/picture_bed/img1/Snipaste_2026-03-06_11-18-15.png)
...
单个时序数据缺乏语义，很容易根据相邻数据推断出来，大量的建模能力被浪费在了逐点学习上面。patchtst通过把一段连续的时序数据作为patch来解决这个问题。然后patchtst是通道独立的，patchtst证明通道独立要更好些。但是简单的通道concate，可能会在trans的初始层中产生噪声交互，从而使得在输出端解开他们变的困难。CrossFormer也利用了patch，而且改进了**通道混合**技术，但是也面临这个问题。
...
- 时间分布漂移：最近的研究主要以**隐式方式**（什么是隐式？SparseTSF: Modeling Long-term Time Series Forecasting with 1k Parameters，itransformer，timeMixer，FITS: Modeling Time Series with 10k Parameters）解决异构时间模式，损害了预测准确性
...
当$\alpha$=1，论文消融实验显示，**仅用频域损失已经能大幅超越传统 DF**

最优$\alpha$通常在 0.8~1 之间，论文超参数敏感性分析显示，$\alpha$在 0.2~1 的宽范围内都能稳定提升模型性能，鲁棒性极强。
...
MTS在许多场景都有应用。MTS核心挑战是对变量内建模的同时也要捕获变量之间的相关性。目前大部分的方法主要集中在捕获变量内（时间建模）。此外，MTS 预测方法的另一个分支已被开发出来，不仅可以对时间依赖性进行建模，而且还强调空间相关性。最具代表性的方法是新兴的基于图神经网络（GNN）的方法[10,2,11]，它们在 MTS 预测任务中取得了最先进的性能。
...
时间序列预测在诸如交通流量、产品销售和能源消耗等领域具有重要价值，因为准确的预测可以使决策者积极主动地进行计划。

为了捕捉这些长期的时间依赖关系，建模变得更加复杂。例如，基于Transformer的模型往往拥有数百万甚至上千万的参数，限制了其实际可用性，尤其是在计算资源受限的场景。

准确的长期时间序列预测的基础在于数据内在的周期性和趋势性。例如，由于此类数据具有明确的日和周模式，因此对家庭电力消费的长期预测是可行的。特别是对于日模式，如果我们将一天中某一时刻的用电量重采样为日序列，则每个子序列表现出相似或一致的趋势。在这种情况下，对原始序列的周期性和趋势性进行分解和变换。也就是说，周期模式被转化为子序列间的动态，而趋势模式被重新解释为子序列内的特征。（idea来源）

核心洞察是：**时间序列的长期预测基础在于数据固有的周期性和趋势**。

原始序列 = 周期成分 P(t) + 趋势成分 T(t)
...
主实验：多变量预测
效果优势：参数量、MACs（乘累加运算）、最大内存、epoch Time。
消融实验和分析：
- 稀疏技术的有效性（将稀疏技术和transformer，GRU， Linear等模型结果，提升其效果）、
- 稀疏技术的表示学习（和线性模型对比，通过可视化权重，说明sparse有更强的周期特征提取能力）
- 超参数w的影响：w是先验周期（和CycleNet相似），给出表格说明设为多少时最佳。这部分说明了，大多数数据集都是有周期性的，给这些数据集设定合适的w是可行的。但是部分数据集没有周期，sparsetst可能不是**首选**，在5.1小节讨论。**但是论文在5.1小节没有讨论这个问题，说的局限是超长周期和多周期，说明sparse在没有周期的数据集上面表现是很差的**。

# SparseTSF: Lightweight and Robust Time Series Forecasting via Sparse Modeling

## 背景

时序列预测在诸如交通流量、产品销售和能源消耗等领域具有重要价值，因为准确的预测可以使决策者主动地计划[ 1 ]，[ 2 ]，[ 3 ]，[ 4 ]。近年来，人们对长期时间序列预测( Long-term Time Series Forecasting，LTSF )越来越感兴趣，它要求模型为高级规划提供扩展的预测视图[ 20 ]，[ 21 ]，[ 22 ]，[ 23 ]，[ 24 ]。

（**指出现有研究的不足，并引出轻量建模**）虽然更长的预测范围提供了便利，但也引入了更大的不确定性[ 25 ]。这就要求模型能够从更长的历史窗口中提取更广泛的时间依赖关系。因此，为了捕捉这些长期的时间依赖关系，建模变得更加复杂。

（**论文的idea来源，通过可视化**）事实上，准确的长期时间序列预测的基础在于数据内在的周期性和趋势性[ 28 ]，[ 29 ]，[ 30 ]，[ 31 ]，[ 32 ]。例如，由于此类数据具有明确的日和周模式，因此对家庭电力消费的长期预测是可行的。特别是对于日模式，将每天特定时间的用电量重采样到日序列中，得到的子序列显示出相似或一致的趋势，如图1所示。在这种情况下，对原始序列的周期性和趋势性进行分解和变换。也就是说，日周期模式被转化为子序列间动态，而趋势模式被重新解释为子序列内特征。这种分解为设计轻量级LTSF模型提供了一个新的视角。


## 和SparseTSF的区别联系

TPAMI等顶刊允许会议论文扩展，但必须满足：
✅ 至少30-50%的新内容
✅ 实质性的理论/方法/实验扩展
...
| 扩展类型 | 具体内容 | 是否实质性 |
|---------|---------|-----------|
| 方法扩展 | 新增MLP变体，解决高维多变量场景 | ✅ 是 |
| 理论扩展 | 隐式正则化证明（全新定理） | ✅ 是（核心贡献） |
| 实验扩展 | 2个新数据集+单变量+泛化+消融 | ✅ 是 |
| 分析深度 | CI/CD策略讨论、超参数分析 | ✅ 是 |


这篇期刊版的核心新贡献不是"Linear变MLP"，而是：

1. 理论贡献：首次证明Sparse技术是结构化隐式正则化
   - 这解释了模型为什么鲁棒
   - 为后续研究提供理论基础

2. 方法贡献：MLP变体解决了原方法的关键局限性
   - 会议版在高维多变量场景表现受限
   - 期刊版通过非线性扩展解决了这个问题
3. 实证贡献：全面的泛化能力分析
   - 4种跨域迁移场景验证
   - 证明模型在小样本、低质量数据场景的适用性


简单说：会议版是"提出想法并初步验证"，期刊版是"深入理论分析并完善方法"。两者是递进关系，而非简单重复。

# TimeBase: The Power of Minimalism in Efficient Long-term Time Series Forecasting

## 背景

长期时间序列预测( LTSF )在能源管理、交通事故预防、极端灾害预警等领域得到了广泛的研究。随着深度学习的快速推进，越来越多的模型被提出（相关工作）。它们都使用了数千到数百万个参数来捕获长期依赖关系和预测未来结果。

（**指出现有研究工作的不足**）通常，较高的参数数量会增加模型容量，这可以导致更好的预测性能( Zhou et al . , 2023c ; Zhao et al , 2024)。在计算机视觉( CV )和自然语言处理( NLP )领域，大型模型取得了显著的成功( He et al . , 2016 ; Liu et al . , 2023)。例如，视觉转换器（vit）。然而，尽管这些模型在特定的预测任务上表现出令人印象深刻的性能，但它们面临着高昂的计算成本和资源密集型要求。

（**承接上文，铺垫下文**）虽然图像和文本作为高维数据依赖各种复杂关系和复杂的底层物理规律，但是如图，一维时间序列表现出明显的时间模式，而且，在长时间序列数据中，这种规律性甚至可以表现为低秩特征( Liu et al . , 2012)，通过奇异值分解可以体现出来，如图1 ( b )所示，这表明存在相当数量的冗余信息( Jones和Brelsford , 1967 ;霍赫赖特和施米德胡贝, 1997)。这就提出了一个重要的问题：**是否真的有必要使用如此大量的参数来学习这些有规律的时间序列模式**？

参数量比SparseTSF还要小几倍，效果要比它更好。

## 实验

1. 长时间步预测（选取的是sparseTSF的linear版本）
2. 性能分析
   - 主要效率比较：可训练参数、MACs、最大内存、epoch time、推理时间（和sparseTSF相比，基本上都下降了）
   - 在超长输入条件下的效率
   - 在超大规模数据集上的效果
3. 在patchtst上即插即用，减少复杂度
4. Timebase积分的量化分析（个人方法的分析）
5. 超参数分析
...
长期时间序列预测 (LTSF) 对于具有长期序列的应用至关重要，这带来了重大挑战，包括长期时间依赖性和面向频率的全局动态。根据 CrossGNN [35] 的说法，像 Transformer 这样基于 DNN 的方法会受到噪声信号的影响，并对意外噪声产生较高的注意力分数。


基于傅里叶的时间序列建模作为一种新的范式，通过在不同频率层次上分层考虑不同的影响来去除噪声信号。如果从基函数的角度重新思考傅里叶变换，实部和虚部可以分别解释为余弦基函数和正弦基函数在分层频率下的系数。

然而，现有的基于傅里叶的方法没有涉及基函数，因此不能准确地解释频率系数，也没有充分考虑时频关系

# FRNet: Frequency-based Rotation Network for Long-term Time Series Forecasting

## 背景

长期时间序列预测( LTSF )是根据过去的观测值估计未来一段时间内的数据点。它在交通预测[ 7 ]，电力负荷预测[ 53 ]，能源管理[ 13 ]和金融[ 49 ]等各个领域都有广泛的应用。

研究[DLinear, 42]表明，时间序列的周期性是LTSF的主导因素，仅通过学习主导周期就可以获得很好的性能。

**现有研究虽然取得了较好的成果，但在对动态复杂周期进行建模方面仍存在不足**。一方面，它们大多只考虑了固定时间步长的静态周期的情况，不具有实用性。随着时间的推移，时间序列的周期并不是严格静态的。实时序列可能存在多个周期[ 3、39]，更重要的是，每个周期成分随时间动态演化。例如，我们从Traffic数据集[ 16 ]中分析了一些案例，并在图1中进行了说明。上图显示了连续三天的交通占有率的下降，这表明连续期间的振幅模式可能会出现波动。下图显示了周期相位的偏移，其中峰值时间较早地出现在第二天，从而表明在这些周期内存在频率偏移。这种动态演化周期的情况广泛存在于各种时间序列数据中[ 3 ]。

频域比时域更能反应周期。我们对现有的旋转方法进行了深入的研究，包括自耦变压器[ 43 ]和Quatformer [ 3 ]。我们的分析表明，这些方法并不能有效地在时间域上表示时间序列数据。首先，时域表征无法有效区分随时间变化的弱动态演化过程。其次，数据中的高频噪声会破坏周期的时域形态，对通过旋转对周期进行动态建模造成干扰。我们提出的想法认为，每个完整周期的子序列在频域中可以更贴切地表示为实部和虚部，同时封装了相位和幅度信息。

相比于时间域，频率域能更好地反映时间序列的周期变化。因此，我们提出在频域中利用旋转来探索动态复杂周期的特征。
