...
![](https://cdn.jsdelivr.net/gh/gaofeng-lin/picture_bed/img1/20210308123113714.png)


TCP/IP是Transmission Control Protocol/ Internet Protocol（传送控制协议/互联网协议）由美国国防部在1970年代开发，基于已有协议（如IP、TCP）设计，优先考虑互联网的实际需求。


OSI是The Open System Interconnection的缩略形式。OSI模型是一个逻辑和概念模型，由ISO（国际标准化组织）于1984年提出，旨在制定通用的网络互联标准。


**现代互联网的HTTP、FTP、DNS等协议均基于TCP/IP模型，而非OSI模型。**
...
一般都是七层模型，如果简单的划分，四层就可以。从上至下分别是：应用层、传输层、网络层、网络接口层
...
**网络接口层(链路层)**：对网络层进行封装，找到传输的设备，嵌入MAC地址。同一局域网内​​通过物理地址（MAC地址）直接通信。

**当设备A向设备B发送数据时，IP地址负责将数据包路由到目标网络，而MAC地址确保数据帧在目标网络内准确传递**
