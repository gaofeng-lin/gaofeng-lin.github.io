...
一般可以通过https或ssh来访问github。如果从linux服务器访问，因为某些原因，无法直接通过https来访问github。所以可以采用ssh访问。

要使用ssh访问，必须用**本地计算机**的私钥文件进行身份验证。也就是说，换一台设备，要使用ssh访问github，也要配置密钥对。


## 生成新的ssh密钥
我们在重装系统后，如果要使用ssh来拉去代码，就需要生成密钥。
[github官方文档](https://docs.github.com/zh/authentication/connecting-to-github-with-ssh/generating-a-new-ssh-key-and-adding-it-to-the-ssh-agent)
...
3.```eval "$(ssh-agent -s)"```；在windows中，是用管理员权限打开PowserShell，输入
```
...
//确保 ssh-agent 正在运行
4.Linux:```ssh-add ~/.ssh/id_ed25519```

Windows:```ssh-add c:/Users/YOU/.ssh/id_ed25519```
